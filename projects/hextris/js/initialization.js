...
function initialize(a) {
	...
	window.op = 0;
	window.saveState = localStorage.getItem("hex-saveState") || "{}";
	if (saveState !== "{}") {
		op = 1;
	}
	...
	highscores = [];
	if (localStorage.getItem('hex-highscores')) {
		try {
			highscores = JSON.parse(localStorage.getItem('hex-highscores'));
		} catch (e) {
			highscores = [];
		}
	}
	...
	if (a != 1) {
		...
		$(window).unload(function() {
			...
			if (gameState == 1 || gameState == -1 || gameState === 0) localStorage.setItem("hex-saveState", exportSaveState());
			else localStorage.setItem("hex-saveState", "{}");
		});
